...
- **include パス補完** — `include '` の入力中に、解決基準ディレクトリ配下のフォルダと `.ttl` ファイルをサジェスト。フォルダ選択時は自動で続きを再サジェスト。include 先が見つからない場合は警告し、似た名前の既存 `.ttl` への置換クイックフィックスを提示（`ttl.diagnostics.includeNotFound`）
...
- **診断（エラー/警告）** — 無効な演算子（`&&`/`++`/`+=` など）、システム変数（`result` など）への代入、条件式での単独 `=`（比較は `==` を推奨）、ブロックの閉じ忘れ（`endif`/`next` など）、深すぎるネスト（既定 2 段）、未知のコマンド（近いコマンドを提案）、未定義ラベルへの `goto`/`call`（include 先も解決）、重複したラベル定義、解決できない `include` 先（似た名前の既存ファイルを提案）を検出
