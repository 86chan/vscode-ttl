...
export const TTL_COMMANDS: ReadonlyArray<TtlCommand> = [
  ...
  },
  {
    ...
    returnsJa: '指定バイト数を受信すると `inputstr` に格納し `result` に 1、タイムアウトで 0。',
    returns: 'On receiving the given number of bytes, stores them in `inputstr` and sets `result` to 1; 0 on timeout.',
  },
  {
    name: 'waitrecv',
  ...
  },
  {
    ...
    returnsJa: '`result` … 置換成功で 1、不一致で 0、書式エラーで -1。`matchstr` にマッチ文字列。`groupmatchstr1`〜`groupmatchstr9` はクリアされる。',
    returns: '`result` … 1 on a successful replace, 0 if no match, -1 on a regex error. `matchstr` holds the matched string; `groupmatchstr1`–`groupmatchstr9` are cleared.',
  ...
  },
  {
    ...
    returnsJa: '`result` … 文字列が見つかると 1（ファイルポインタは直前へ移動）、見つからないと 0（移動なし）。',
    returns: '`result` … set to 1 if the string is found (the file pointer moves just before it), 0 if not (pointer unchanged).',
  ...
  },
  {
    ...
    returnsJa: '`result` … 取得成功で 1、配列の要素数が足りない場合 0、取得失敗で -1。',
    returns: '`result` … 1 on success, 0 if the array has too few entries, -1 on failure.',
  },
  {
    name: 'getipv6addr',
    ...
    returnsJa: '`result` … 取得成功で 1、配列の要素数が足りない場合 0、取得失敗で -1（Windows 2000 以前では常に失敗）。',
    returns: '`result` … 1 on success, 0 if the array has too few entries, -1 on failure (always fails on Windows 2000 or earlier).',
  },
  {
    name: 'getmodemstatus',
    ...
    returnsJa: '`result` … 読み出しに成功すると 0、失敗すると 1。',
    returns: '`result` … set to 0 when the status is read successfully, 1 on failure.',
  ...
  },
  {
    ...
    returnsJa: '`result` … 取得に成功すると 1、失敗すると 0。',
  ...
  },
  {
    ...
    returnsJa: '`result` … `<wait>` が 1 なら -1=実行失敗 / それ以外=終了コード。`<wait>` が 1 以外なら -1=失敗 / 0=成功。*(4.103 以降)*',
    returns: '`result` … if `<wait>` is 1: -1=failed to run / otherwise the exit code. If `<wait>` is not 1: -1=failed / 0=success. *(4.103 or later)*',
  ...
  },
  {
    ...
    returnsJa: '`result` … 0=クリップボードを開けない/テキストでない/offset 不正, 1=代入成功, 2=代入したが切り捨てあり, 3=メモリ確保失敗。',
    returns: '`result` … 0=cannot open clipboard / not text / bad offset, 1=stored, 2=stored but truncated, 3=memory allocation failed.',
